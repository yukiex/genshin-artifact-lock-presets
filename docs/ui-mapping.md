...
| HP（実数） | `HP` | フラット値 |
| 防御力（実数） | `DEF` | フラット値 |
| 元素ダメージ%（炎/水/雷/風/氷/岩/草） | `<Element>DMG%` | 例: `PyroDMG%`, `ElectroDMG%`, `DendroDMG%`。UIの「以下のオプションをすべて選択」ボタンには物理ダメージも含まれるため、YAML では元素7種を列挙し、物理を許可する場合は `PhysicalDMG%` を追加する。 |
...
> **補足1**: UI では「以下のオプションをすべて選択」で元素/物理の両方を一括指定できるが、YAML では各元素（`PyroDMG%`〜`DendroDMG%`）と物理（`PhysicalDMG%`）を明示して `main_allowed` に並べること。
>
> **補足2 – 元素ダメージ表記対応表**
> | UIの表記 | YAMLキー |
> | --- | --- |
